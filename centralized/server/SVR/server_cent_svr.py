...
class SVR(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.linearModel=nn.Linear(in_features,out_features)
...
def hingeLoss(outputVal,dataOutput,model):
    loss1=torch.sum(torch.clamp(1 - torch.matmul(outputVal.reshape(outputVal.shape[0] , outputVal.shape[2], outputVal.shape[1]) ,dataOutput),min=0))
...
while iteration < args.communication_rounds - 1:
    ...
    for epoch in range(args.epochs):
        for x_batch, y_batch in train_loader:
            ...
            optimizer.zero_grad()
            modelOutput = model(x_batch)
            totalLoss= hingeLoss(y_batch,modelOutput,model)
    
            trainTotalLoss +=  totalLoss
        ...
        train_losses.append(trainTotalLoss/len(train_loader.dataset))
                
        for x_val, y_val in valid_loader:
            output = model(x_val)
            valLoss = hingeLoss(output,y_val, model)
            
            valAllLoss += valLoss
        
        val_losses.append(valAllLoss/len(valid_loader.dataset))
